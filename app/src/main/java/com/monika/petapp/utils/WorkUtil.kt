...
object WorkUtil {
    ...
    // Return start and end of work
    private fun getStartEndTimeHours(input: String): Pair<String, String> {
        val p: Pattern =
            Pattern.compile("^[A-Z]-[A-Z]\\s{1}(\\d{1,2}:\\d{2})\\s{1}-\\s{1}(\\d{1,2}:\\d{2})$")
        val m: Matcher = p.matcher(input)
        val items = ArrayList<String>()
        while (m.find()) {
            m.group(1)?.let { items.add(it) }
            m.group(2)?.let { items.add(it) }
        }
        return Pair(items[0], items[1])
    }

    // Return if current working time is valid or not
